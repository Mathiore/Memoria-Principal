...
int main() {
    setlocale(LC_ALL, "");
    string filename = "addresses_32b.txt"; // Nome do arquivo de texto
    ...
    int lineIndex;
    cout << "Digite o índice da linha a ser lida: ";
    cin >> lineIndex;
    ...
    string binaryRepresentation = bitset<32>(chosenNumber).to_string();

    // Calcular o número da página e o deslocamento
    string pageBinary = binaryRepresentation.substr(0, 20);  // 4 primeiros caracteres para o número da página
    string offsetBinary = binaryRepresentation.substr(20);    // Restante para o deslocamento

    int pageNumber = bitset<20>(pageBinary).to_ulong();      // Converter binário para decimal
    ...
    int value;
    int pageSize = 4096; // Tamanho da página de 256 bytes para endereços de 16 bits
